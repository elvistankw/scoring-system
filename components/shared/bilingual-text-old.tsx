...
export function BilingualText({
  ...
  const getTranslation = (key: string, targetLocale: 'zh' | 'en') => {
    ...
    return translations[key]?.[targetLocale] || t(key);
  };
  ...
  if (respectLocale && locale === 'zh') {
    return (
      <span className={`${chineseSize} ${className}`}>
        {getTranslation(translationKey, 'zh')}
      </span>
    );
  }
  ...
  const chineseText = getTranslation(translationKey, 'zh');
  const englishText = getTranslation(translationKey, 'en');
